...
const ListVeterinarios: React.FC = () => {
  ...
  const fetchVeterinarios = async () => {
    setLoading(true);
    try {
      const response = await authFetch("https://168.231.88.35:8080/veterinarios");
  ...
  const onRowEditComplete = async (e: DataTableRowEditCompleteEvent) => {
    ...
    try {
      const response = await authFetch(`https://168.231.88.35:8080/veterinarios/${updatedVeterinario.id}`, {
        method: "PUT",
  ...
  const deleteVeterinario = async (id: number) => {
    try {
      const response = await authFetch(`https://168.231.88.35:8080/veterinarios/${id}`, { method: "DELETE" });
